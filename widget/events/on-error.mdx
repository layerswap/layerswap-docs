...
<Note>
  This callback reports errors **inside a loaded, running widget**. Failures to load the widget itself (network, manifest, signature issues) surface through the top-level `onError` prop instead — see [How It Works & Security](/widget/how-it-works#two-error-channels).
</Note>
