...
description: "Diagnose a blank or stuck Layerswap Widget: read the error, check your CSP, recover."
---
...
The production origin is `https://cdn.layerswap.io`. Copy the complete policy from [How the Widget works](/widget/how-it-works#recommended-content-security-policy).

<Warning title="Verify at release — M18">Confirm that the released loader actually fetches its manifest from `cdn.layerswap.io` and test the documented CSP in a production-like host.</Warning>

## Widget renders an alert instead of the UI

"Only one Layerswap widget can be live per page" — you mounted two (any combination of swap and deposit widgets). Render one at a time. In the JavaScript package the second `mountWidget` call throws instead.
...
<Warning title="🚧 Needs review — support link placeholder (Q3.8)">Official support channel link pending.</Warning>
