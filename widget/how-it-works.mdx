...
description: "How the CDN-delivered Widget is loaded and verified, including manifest signing, integrity checks, CSP, and failure modes."
---
...
```mermaid
flowchart LR
  A[Loader mounts] --> B[Fetch signed manifest]
  B --> D[Verify P-256 signature and freshness]
  D --> E[Register SHA-384 integrity map]
  E --> F[Load Module Federation remote]
  F --> G[Render Widget]
```
...
The manifest channel is release policy rather than a runtime compatibility check. Widget packages are expected to ship on the 2.0.0 line together; verify the final channel path on release day.

<Warning title="Verify at release">
  Confirm that the released loader resolves the expected 2.x manifest channel and that `https://cdn.layerswap.io` serves it successfully.
</Warning>
...
<Warning title="Release blocker — ENG-1">
  Engineering must change the loader's `DEFAULT_MANIFEST_URL` to `cdn.layerswap.io` before publication. The documentation and CSP are intentionally written against the production origin.
</Warning>
