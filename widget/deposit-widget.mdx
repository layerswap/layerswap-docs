...
- users may hold funds in different wallets, exchanges, or supported app balances;
- you want Layerswap to present only the funding paths available for that destination; and
- the experience should render inline with your product or open from a branded button.

Use the [Full Swap Widget](/widget/quickstart) when users need to choose the destination themselves. Use the [API](/api/quickstart) when you need to replace the method picker or own the complete funding interface.
...
- **Deposit from Hyperliquid** — when the destination and runtime support it, the user can fund the deposit from their Hyperliquid balance without copying a destination address or leaving the widget.
...
<Warning>
  **One Layerswap widget per page** — of either kind. Don't render `LayerswapDepositWidget` and `LayerswapWidget` on the same page at the same time; swap between them instead.
</Warning>

### Choosing which funding methods to show

By default every available method is offered. Pass `methods` as an allow-list to narrow them — only the listed methods can appear, and each still needs its own runtime conditions (e.g. `hyperliquid` needs a destination reachable from Hyperliquid). For a deposit-address-only integration with no wallet connection at all:
...
<ResponseField name="methods" type="DepositMethodId[]" default="all methods">
  Allow-list of funding methods the picker may show — `"wallet"`, `"deposit_address"`, `"hyperliquid"`, and `"polymarket"`. Only listed methods can appear, and each still needs its own runtime conditions to be offered. Defaults to every method.
</ResponseField>

<Warning title="🚧 Needs review — method descriptions (M16)">
  Product still needs to supply the user-facing one-line description for the `polymarket` flow. Its identifier is public and supported, but this reference does not guess at the UX.
</Warning>
