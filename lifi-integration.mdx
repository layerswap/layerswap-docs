...
| LI.FI expects | Layerswap field | Notes |
|---|---|---|
| `status` | `swap.status` | See [Swap Lifecycle](/concepts/swap-lifecycle) for full details. Values: `user_transfer_pending`, `ls_transfer_pending`, `completed`, `failed`, `expired`, `pending_refund`, `refunded` |
