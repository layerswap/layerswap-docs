...
- [@tonconnect/ui-react](https://github.com/nicolo-ribaudo/tonconnect-ui-react) (for browser wallets) or a TON SDK for server-side signing
...
- `amount` — Present only for Jetton transfers; the Jetton amount in base units (nanojettons). For native TON transfers, use the deposit action's `amount` field instead.
