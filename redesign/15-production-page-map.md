...
| 39 | `api/funding/transfer` — Fund by transfer | Reference·Guide / C | **CREATE**; promote BTC OP_RETURN / Solana-memo / EVM-calldata mechanics out of hidden `lifi-integration.mdx` (fixes CMP-4) | Wallet-transfer and deposit-address funding: reading `deposit_actions`, per-chain-family construction, wrong-amount behavior. | lifi-integration (mechanics only), capability §7, spec | 🚧 Solana memo program id (backend check, U-15b); 🚧 `deposit_speedup` semantics |
...
| 53 | `lifi-integration` (hidden tab) | KEEP hidden as-is (Q3.3); deposit-construction mechanics promoted to `api/funding/transfer` (content copy, page untouched) | Team hasn't decided its fate; it's the only home of partner-specific framing. |
