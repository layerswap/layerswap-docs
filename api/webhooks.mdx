...
<Warning title="🚧 Needs review — webhook contract (Q2.5)">
  The complete event list, exact envelope and field guarantees, retry schedule, timeout behavior, duplicate-delivery policy, and ordering guarantees are not documented. Build an idempotent receiver, tolerate partial fields, and re-fetch the swap until the contract is published.
</Warning>
