---
title: "Build your first API swap"
description: "One complete flow: authenticate, quote a route, create the swap, fund it, and track it to completion — with the failure cases at each step."
---

By the end of this page you'll have moved ETH from Ethereum to Arbitrum through the API: quote → create → fund → track. Every response shape below comes from the live API.

```mermaid
sequenceDiagram
    participant Y as Your backend
    participant L as Layerswap API
    participant W as User's wallet
    Y->>L: GET /quote (route + amount)
    L-->>Y: quote (fees, min receive)
    Y->>L: POST /swaps
    L-->>Y: swap id + deposit_actions
    Y->>W: request transfer per deposit_actions
    W->>L: on-chain deposit detected
    L-->>Y: status: ls_transfer_pending → completed
```

## Before you start

You need an API key from the [Partner Dashboard](/get-started/api-keys). Keys are per-environment: **a testnet key returns testnet routes, and a mainnet key returns mainnet routes from the same base URL**.
...
**If auth fails:** an _invalid_ key returns `403` with `{"error": {"code": "API_KEY_FORBIDDEN", "message": "Api key forbidden"}}`. A _missing_ key silently gives you public mainnet access — don't mistake it for a working testnet setup.
...
<Steps>
  ...
  <Step title="Get a quote">
    ...
    <Warning title="🚧 Needs review — awaiting API answer (Q2.1)">
  ...
  <Step title="Create the swap">
    ...
    <Warning title="🚧 Needs review — awaiting API answer (Q2.7)">
    ...
    <Warning title="🚧 Needs review — awaiting API answer (Q2.4)">
  ...
  <Step title="Fund the swap">
    ...
    **If the user sends an invalid amount:** inspect `fail_reason`; the swap may enter the [refund flow](/api/refunds). **If the user never sends:** the swap moves to `expired` after six hours. See [Swap lifecycle & statuses](/concepts/swap-lifecycle) instead of inferring recovery behavior.
  </Step>
  <Step title="Track to completion">
    ...
    <Warning>
      Recommended polling cadence and webhook delivery semantics are unconfirmed. Poll conservatively, back off on failures, and configure [Webhooks](/api/webhooks) in the Partner Dashboard.
    </Warning>
  </Step>
</Steps>
