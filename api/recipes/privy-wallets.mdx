...
description: 'Fund a Layerswap swap from a Privy server wallet with a sponsored Depository call.'
---

This flow funds an ERC-20 Layerswap swap from a Privy server wallet. It creates a swap with `use_depository: true`, batches the token approval with the Depository call, and submits both through Privy's gas sponsorship, so the wallet does not need native gas. See the [full example on GitHub](https://github.com/layerswap/examples/tree/main/privy-wallets).
...
<Steps>
  ...
  <Step title="Check the swap status">
    ...
    This maps the final transaction back to the Layerswap swap record. See [Swap lifecycle & statuses](/concepts/swap-lifecycle) for the state machine.
  </Step>
</Steps>
