---
title: "Tab Options"
description: "Configuring the Swap, Deposit from CEX, and Easy Deposit flows in Layerswap Widget"
---
...
<LayerswapProvider
  config={{
    ...
    version: 'mainnet',
    initialValues: {
      defaultTab: 'swap' // 'swap' | 'cex' | 'deposit' (alias: 'easy')
    }
  }}
...
<ResponseField name="defaultTab" type="string">
  ...
  - `'deposit'` — Easy Deposit flow (`'easy'` is accepted as an alias)
</ResponseField>
...
The Easy Deposit flow gives the user a deposit address they can send to from any wallet — no WalletConnect, no extension popup, no source-side wallet integration. Enable it with `defaultTab: 'deposit'` (or `'easy'`).
