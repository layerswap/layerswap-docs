---
title: "Layerswap Widget Integration Guide for Starknet"
...
Install the Layerswap Widget along with the necessary wallet providers for Starknet:
...
export default function useStarknet({
  ...
  const connectWallet = useCallback(async (): Promise<Wallet | undefined> => {
    // Log out existing wallet if any
    if (userWallets.length) {
      await handleLogOut();
    }

    // Show Dynamic auth flow and wait for wallet connection
  ...
  // Map Dynamic SDK wallets to Layerswap Wallet format
...
/** Helper function to convert DynamicWallet to Layerswap Wallet */
...
<Note>
  **Get Your API Keys:**
  ...
  - Layerswap: Generate from the [Partner Dashboard](/api-reference/api-keys)
</Note>
...
<iframe
  src="https://layerswap-widget-nextjs-example.vercel.app/"
  style={{
    width: '100%',
    height: '700px',
    border: '0',
...
<Note>
  This demo includes a complete working implementation with Starknet wallet integration using Dynamic Labs SDK. You can explore code at this [repository](https://github.com/arentant/layerswap-widget-nextjs-example).
</Note>
